...
int main (int argc, char *argv[]){
  ...
  Mat frame, frame_HSV, frame_threshold, test(Size(frame.cols,frame.rows),CV_8UC3);

  while (true){
      cap >> frame;
      if (frame.empty ()){break;}
      cvtColor (frame, frame_HSV, COLOR_BGR2HSV, 0);
      inRange (frame_HSV, Scalar (1, 3, 130), Scalar (46, 127, 245),frame_threshold);
      for (int i = 0; i < test.rows; i++){// NbRow = 480 (0-479)
	  for (int j = 0; j < test.cols; j++){// NbCols = 640 (0-639)
