/* *************** Stereo Camera Calibration **************************
 ...
 --images, -n (value:20)                    No of stereo pair images
...
// Ecriture sur image du nombre de capture fait / nombre de capture total
Mat displayCapturedImageIndex(Mat img)
{
    ostringstream imageIndex;
    imageIndex << stereoPairIndex << "/" << noOfStereoPairs;
    putText(img, imageIndex.str().c_str(), Point(50, 70),
	    FONT_HERSHEY_PLAIN, 0.9, Scalar(0, 0, 255), 2);
    return img;
}

// Ecriture sur l image du mode en cours
Mat displayMode(Mat img)
{
    ...
    putText(img, modeString, Point(50, 50), FONT_HERSHEY_SIMPLEX, 0.5,
	    Scalar(0, 255, 0), 2);
    if (mode == CAPTURING) {
...
// Recherche coin dans les images de camera gauche et droite
bool findChessboardCornersAndDraw(Mat inputLeft, Mat inputRight)
{
    ...
    foundLeft =
	findChessboardCorners(inputLeft, boardSize, cornersLeft,
			      CALIB_CB_ADAPTIVE_THRESH |
			      CALIB_CB_NORMALIZE_IMAGE);
    foundRight =
	findChessboardCorners(inputRight, boardSize, cornersRight,
			      CALIB_CB_ADAPTIVE_THRESH |
			      CALIB_CB_NORMALIZE_IMAGE);
    // Dessine sur l'image gauche
    drawChessboardCorners(_leftOri, boardSize, cornersLeft, foundLeft);
    // Dessine sur l'image droite
...
// Affichage des images sur l ecran
void displayImages()
{
...
// Sauvegarde les images prisent pour la calibration
void saveImages(Mat leftImage, Mat rightImage, int pairIndex)
{
	...
	leftString << dir << "/" << prefixLeft << pairIndex << "." <<
	    postfix;
	rightString << dir << "/" << prefixRight << pairIndex << "." <<
	    postfix;
...
{
    // Construction de tableau 2D à 3 float
    vector < vector < Point3f > >objectPoints;
    objectPoints.resize(noOfStereoPairs);
    // Parcours les points[j][k] du nm de pair stereo
    for (int i = 0; i < noOfStereoPairs; i++) {
	for (int j = 0; j < boardSize.height; j++) {
	    for (int k = 0; k < boardSize.width; k++) {
		objectPoints[i].
                        push_back(Point3f(float (j), float (k), 0.0));
	    }
	}
    }
    ...
    Mat R, T, E, F;
    double rms =
	stereoCalibrate(objectPoints, cameraImagePoints[0],
			cameraImagePoints[1],
	...
	for (int j = 0; j < npt; j++) {
	    double errij =
		fabs(cameraImagePoints[0][i][j].x * lines[1][j][0] +
		     cameraImagePoints[0][i][j].y * lines[1][j][1] +
		     lines[1][j][2]) +
		fabs(cameraImagePoints[1][i][j].x * lines[0][j][0] +
		     cameraImagePoints[1][i][j].y * lines[0][j][1] +
		     lines[0][j][2]);
	    err += errij;
	}
	npoints += npt;
	...
	for (int j = 0; j < 2; j++) {
	    ...
	    cimg = rimg;
	    Mat canvasPart =
		!isVerticalStereo ? canvas(Rect(w * j, 0, w, h)) :
...
void calibrateInRealTime(int cam1, int cam2)
{
	...
	camLeft >> inputLeft;
	camRight >> inputRight;
	if ((inputLeft.rows != inputRight.rows)
	    || (inputLeft.cols != inputRight.cols)) {
	    cout <<
		"Error: Images from both cameras are not of some size. Please check the size of each camera.\n";
	    exit(-1);
	}
	inputLeft.copyTo(copyImageLeft);
	inputRight.copyTo(copyImageRight);
	foundCornersInBothImage =
	    findChessboardCornersAndDraw(inputLeft, inputRight);
	if (foundCornersInBothImage && mode == CAPTURING
		...
		goIn = 0;
		saveImages(copyImageLeft, copyImageRight,
			   ++stereoPairIndex);
...
void calibrateFromSavedImages(string dr, string prel, string prer,
			      string post)
{
    Size imageSize;
    for (int i = 0; i < noOfStereoPairs; i++) {
	...
	sourceLeftImagePath =
	    dr + "/" + prel + imgIndex.str() + "." + post;
	sourceRightImagePath =
	    dr + "/" + prer + imgIndex.str() + "." + post;
	...
	if (inputLeft.empty() || inputRight.empty()) {
	    cout << "\nCould no find image: " << sourceLeftImagePath <<
		" or " << sourceRightImagePath << ". Skipping images.\n";
	    continue;
	}
	if ((inputLeft.rows != inputRight.rows)
	    || (inputLeft.cols != inputRight.cols)) {
...
int main(int argc, char **argv)
{
	...
	"{cam1|0|Camera 1 Index}" "{cam2|2|Camera 2 Index}";
	...
	break;
    case 1:
	calibrateInRealTime(parser.get < int >("cam1"),
			    parser.get < int >("cam2"));
	break;
    default:
